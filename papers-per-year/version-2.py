...
for line in range(0, df.shape[0]):
     axes[1].text(df['year'][line]-2, df['size'][line], df["tag"][line], horizontalalignment='left', size='small', color='black', weight='semibold')

axes[1].get_yaxis().set_visible(False)
axes[1].set_xlabel('Year')
axes[1].set_xlim(2010, 2021)
...
axes[2].set_xlim(2000, 2021)
axes[2].set_title("Peak Traffic Rate (Gbps)")

plt.tight_layout()
# plt.show()
plt.savefig('recent-ddos-attacks-papers.pdf')